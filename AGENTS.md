...
- **工具调用**：内置 `bash`、`cmd`、`load_skill` 工具；支持通过 MCP 协议挂载外部工具（配置于 `tools/toolsConfig.json`）。
...
│   └── basetools/           #   内置工具：bash、cmd、load_skill
├── internal/memory/         # 存储层：JSONL 格式会话 & trace 写入/读取
