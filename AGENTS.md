...
cmd/agent-server/main.go     # HTTP Server 入口：多会话并发，监听 /chat 端点
cmd/qq/main.go               # QQ 事件适配器：接收 NapCat OneBot 11 webhook，路由至 agent-server
│
...
QQ 用户
  → NapCat（接收 QQ 消息）
  → POST webhook → cmd/qq（qq-adapter）:8081
      → 解析 OneBot 11 事件，过滤 @bot / 私聊
      → POST /chat → cmd/agent-server（amadeus-agent）:9501
      ...
      → cmd/qq 调用 NapCat HTTP API 发送回复
  → QQ 用户收到回复
```
...
# Agent Server（HTTP 模式）
AGENT_SERVER_ADDR=:9501                         # 监听地址
AGENT_SERVER_URL=http://localhost:9501          # 本地开发时；Docker 环境改为 http://amadeus-agent:9501

# QQ Bot（NapCat）
NAPCAT_API_URL=http://127.0.0.1:3000           # 本地开发时；Docker 环境改为 http://napcat:3000
...
MILVUS_ADDRESS=localhost:19530          # 本地开发时；Docker 环境改为 milvus:19530
MILVUS_COLLECTION=amadeus_memory        # Collection 名称
```

### Docker 部署

项目提供统一 `docker-compose.yml`，一键启动七个服务：

| 服务 | 镜像 | 端口 | 说明 |
|------|------|------|------|
| `napcat` | `mlikiowa/napcat-docker:latest` | 3000/3001/6099 | QQ 框架，6099 为 WebUI |
| `qq-adapter` | `amadeus`（本地构建） | 8081 | QQ 事件适配器（cmd/qq） |
| `amadeus-agent` | `amadeus`（本地构建） | 9501 | AI Agent HTTP Server |
| `milvus` + `etcd` + `minio` | 官方镜像 | 19530 | 向量数据库栈 |
...
# 2. 打开 NapCat WebUI，扫码登录 QQ（仅需一次）
open http://localhost:6099/webui   # 默认 token: napcat

# 3. 在 NapCat WebUI 中配置 HTTP 上报地址为 http://qq-adapter:8081

# 4. 验证服务状态
docker compose ps
```

**NapCat 持久化登录**：`napcat-data/ntqq/` 保存 QQ 登录数据，`mac_address` 固定防止重复扫码，重启后免登录。

**Docker 环境需修改 `.env` 中三处地址：**
```
...
| 宿主机目录 | 容器路径 | 说明 |
|-----------|---------|------|
| `./skills/` | `/app/skills` | Skill 定义，修改后重启生效 |
| `./tools/` | `/app/tools` | MCP 工具配置 |
| `./checkpoints/` | `/app/checkpoints` | 会话历史持久化 |
| `./napcat-data/ntqq/` | `/app/.config/QQ` | QQ 登录数据 |
| `./napcat-data/config/` | `/app/napcat/config` | NapCat 配置 |
...
| Docker 编排 | `docker-compose.yml` | 七服务统一编排（napcat/qq-adapter/amadeus-agent/milvus 栈） |
...
| Docker 编排变化（服务增减、端口、volume） | 本地开发和启动流程 → Docker 部署 |
| 新增 / 删除 `cmd/` 入口 | 架构 → 包结构图、启动命令 |
